...
INT spiFlashEraseBlockNb(UINT32 addr);
int usiCheckBusyNb(void);
...
unsigned int g_UpdateStartBlock, g_EndTagBlock;
    ...
    if (((*(pImageList+0)) == 0xAA554257) && ((*(pImageList+3)) == 0x63594257))
    {
        ...
        for (i=0; i<count; i++)
        {
            if (((*(pImageList) >> 16) & 0xffff) == 1)    /* execute */
            {
                ...
                sysprintf("\rDefault Firmware : %d ~ %d (%d)\n", startBlock, endBlock,fileLen);
                sysprintf("\rUpdate Firmware start : %d\n", g_UpdateStartBlock);
                ...
                if((*(pImageList + 0) == 0x4E565420) && (*(pImageList + 3) == 0x2054564E))
                {
                    ...
                    {
                        g_Version = ver;
                        sysprintf("\rUpdate Firmware (Ver. %d): %d Bytes (End TAG Block %d)\n",g_Version, size,g_EndTagBlock);
                    }
                }
                break;
            }
...
INT32 HID_CmdReadPages(CMD_T *pCmd)
{
    ...
    if(u32Pages)
    {
        ...
        spiFlashRead(u32StartPage * SPI_PAGE_SIZE, g_u32ReadWriteSize , (UINT8 *)g_u8PageBuff);
...
void HID_GetOutReport(void)
{
    ...
    u32Pages     = pCmd->u32Arg2;
    u32PageCnt   = pCmd->u32Signature; /* The signature word is used to count pages for WRITE CMD */    
    ...
    if((u8Cmd == HID_CMD_WRITE) &&  (u32PageCnt < u32Pages))
    {
        ...
        {
            //sysprintf("%d  ", u32StartPage + u32PageCnt);
            /* TODO: We should program received data to storage here */
            spiFlashWrite((u32StartPage + u32PageCnt) * SPI_PAGE_SIZE, g_u32ReadWriteSize, (UINT8 *)g_u8PageBuff);
...
void HID_SetInReport(void)
{
    ...
    if(u8Cmd == HID_CMD_READ)
    {
        ...
        else
        {
            ...
            {
                spiFlashRead((u32StartPage + u32PageCnt) * SPI_PAGE_SIZE, g_u32ReadWriteSize , (UINT8 *)g_u8PageBuff);
                if(g_u32EPA_MXP == EPA_MAX_PKT_SIZE)
                    u32PageCnt+=2;                    
                else        
                {
            ...
            if(g_u32EPA_MXP == EPA_MAX_PKT_SIZE)
                ...
                outp32(AHB_DMA_ADDR, (UINT32)&g_u8PageBuff[SPI_PAGE_SIZE - g_u32BytesInPageBuf]);
            outp32(DMA_CNT, g_u32EPA_MXP);
            outp32(DMA_CTRL_STS, inp32(DMA_CTRL_STS)|0x00000020);
            while(usbdInfo.USBModeFlag)
            {
