...
void init(void)
{
	...
	sysprintf("SPI Loader (UpdateLoader) start - O2DN (%s).\n", DATE_CODE);
#else
	sysprintf("SPI Loader (UpdateLoader) start (%s).\n", DATE_CODE);
#endif	
}
	...
	if (((*(pImageList+0)) == 0xAA554257) && ((*(pImageList+3)) == 0x63594257))
	{
		...
		for (i=0; i<count; i++)
		{
			if (((*(pImageList) >> 16) & 0xffff) == 1)	// execute
			{
                ...
                g_EndTagBlock = 0;
#ifndef __OTP_4BIT__
                SPIReadFast(0, g_UpdateStartBlock * 0x10000, 512, (UINT32*)pImageList);
#else
                spiFlashFastReadQuad(g_UpdateStartBlock * 0x10000, 512, (UINT32*)pImageList);
#endif
                start_addr = startBlock * BLOCK_SIZE;
...
#ifndef __OTP_4BIT__
                    SPIReadFast(0, g_UpdateStartBlock * 0x10000 + offset, 4, (UINT32*)pImageList);
#else
                    spiFlashFastReadQuad(g_UpdateStartBlock * 0x10000 + offset, 4, (UINT32*)pImageList);
#endif
                    if(*pImageList == 0xA55AA55A)
                    {
                        g_Version = ver;
                        sysprintf("Firmware (Ver. %d): %d Bytes\n",g_Version, size);
                        start_addr = g_UpdateStartBlock * BLOCK_SIZE + 16;
                        fileLen = size;
                    }
                }

#ifndef __OTP_4BIT__
                SPIReadFast(0, start_addr, fileLen, (UINT32*)executeAddr);
#else
                spiFlashFastReadQuad(start_addr, fileLen, (UINT32*)executeAddr);
#endif
