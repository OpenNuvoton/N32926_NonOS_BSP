...
INT  W99683_GetFramePiece(UINT8 **pbuf, INT *length)
{
    ...
    while (1)
    {
        ...
        while ((int)_W99683_Camera->vbq_head == (int)_W99683_Camera->vbq_tail)
        {
        }
...
static VOID usbvideo_IsocIrq(URB_T *urb)
{
    ...
    newBuffer = USB_malloc(FRAMES_PER_DESC * _W99683_Camera->iso_packet_len, BOUNDARY_WORD);
    if (newBuffer == NULL) 
    {
    ...
    dummy_vbuf = (USB_VID_BUF_T *)USB_malloc(sizeof(USB_VID_BUF_T), BOUNDARY_WORD);
    if (dummy_vbuf == NULL)
    {
...
static INT  W99683Cam_I2C_read(UINT16 slave_addr, UINT16 index, UINT8 *regb)
{
    ...
    i = USB_SendControlMessage(_W99683_Camera->dev, usb_rcvctrlpipe(_W99683_Camera->dev, 0),
                        ...
                        USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE,
                        slave_addr, index, _dma_data, 1, HZ * 3);
    if (i < 0)
...
static INT  W99683Cam_I2C_write(UINT16 slave_addr, UINT16 index, UINT8 value)
{
    ...
    i = USB_SendControlMessage(_W99683_Camera->dev, usb_sndctrlpipe(_W99683_Camera->dev, 0),
                        ...
                        USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_DEVICE,
                        slave_addr, index, _dma_data, 1, HZ * 10);
    if (i < 0)
...
static INT  W99683Cam_check_sensor(INT *capc1)
{
    ...
    regb = (UINT8 *)USB_malloc(1, BOUNDARY_WORD);
...
INT  W99683Cam_Open()
{
    ...
    _W99683_Camera->sbuf[0].data = USB_malloc(FRAMES_PER_DESC * _W99683_Camera->iso_packet_len, BOUNDARY_WORD);
                          ...
                          const struct usb_device_id *devid)
{
    ...
    _W99683_Camera = (UVD_T *)USB_malloc(sizeof(UVD_T), BOUNDARY_WORD);
    if (_W99683_Camera == NULL) 
    {
    ...
    vbuf = (USB_VID_BUF_T *)USB_malloc(sizeof(USB_VID_BUF_T), BOUNDARY_WORD);
    if (vbuf == NULL)
    {
