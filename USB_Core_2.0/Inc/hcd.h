...
struct hc_driver 
{
	...
	int	(*suspend)(unsigned int state);
	...
	void (*free_config)(struct usb_device *dev);

	/* root hub support */
	int	(*hub_status_data)(char *buf);
	int	(*hub_control)(UINT16 typeReq, UINT16 wValue, UINT16 wIndex, char *buf, UINT16 wLength);
};

extern void usb_hcd_giveback_urb(struct urb *urb);
...
static __inline struct usb_bus *hcd_to_bus (struct usb_hcd *hcd)
	{ return hcd->bus; }
