...
int main()
{
    ...
    Interface_Init();
    // Interface_Wait_Start();  /* Comment and don't support START button */
    Interface_NAND_Power_ON();
    Interface_LED_Y_Toggle();
#endif
    ...
    sicOpen();
    sicIoctl(SIC_SET_CARD_DETECT, TRUE, 0, 0);  /* Enable SD card detection feature. */
    status = sicSdOpen0();
    if (status < 0)
    {
    ...
    status = nvtSMInit();   // initial NAND controller and pNvtSM0
    if (status < 0)
    {
        Draw_Status(font_x+ u32SkipX*g_Font_Step, font_y, Fail);
        sysprintf("===> 1.3 (Wrong NAND Flash ID)\n");
        bIsAbort = TRUE;
        goto _end_;
    }
...
_end_:
    ...
    if (bIsAbort)
        ...
        Interface_LED_G_ON();   // NAND Writer PASS
#endif
