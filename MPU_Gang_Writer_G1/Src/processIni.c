/**************************************************************************//**
 * @file     processIni.c
 * @brief    MPU Gang Writer G1 source code for NandWriter.ini file process
 *
...
int ProcessINI(char *fileName)
{
    ...
    Ini_Writer.SystemReservedMegaByte = 8;
...
NextMark2:
        ...
        else if (strcmp(Cmd, "[NANDCARD FAT FILE]") == 0)
        {
            do {
                status = readLine(FileHandle, Cmd);
                if (status < 0)
                ...
                {
                    Ini_Writer.NANDCARD_FAT = atoi(Cmd);
                    break;
                }
            } while (1);
        }
