 ...
 * @file     PWM.c
 * @version  V3.00
 * @brief    N3292x series PWM driver source file
 *
...
VOID PWM_EnableInt(UINT8 u8Timer, UINT8 u8Int)
{
	...
	{		
		if(u8Timer & 0x02)		
			outp32(CCR1, (inp32(CCR1) & ~(0x06 << ((u8Timer & 0x01) << 4 ))) |	((u8Int & 0x03) << (((u8Timer & 0x01) <<4) + 1)));					
		else
			outp32(CCR0, (inp32(CCR0) & ~(0x06 << ((u8Timer & 0x01) << 4 ))) |	((u8Int & 0x03) << (((u8Timer & 0x01) <<4) + 1)));					
	}
	else
...
{   
	...
	{
		if(u8Timer & 0x02)		
			outp32(CCR1, inp32(CCR1) & ~((u8Int & 0x03) << (((u8Timer & 0x01) <<4) + 1)));					
		else
			outp32(CCR0, inp32(CCR0) & ~((u8Int & 0x03) << (((u8Timer & 0x01) <<4) + 1)));					
...
VOID PWM_ClearInt(UINT8 u8Timer)
{
	...
	{
		if(u8Timer & 0x02)
			outp32(CCR1, (inp32(CCR1) & 0xF000F) | (1 << (((u8Timer & 0x01) << 4) + 4)));
		else
			outp32(CCR0, (inp32(CCR0) & 0xF000F) | (1 << (((u8Timer & 0x01) << 4) + 4)));			
	}
	else
		outp32(PIIR,  inp32(PIIR) & ~(1<<(u8Timer & 0x03)));
}
...
{
	if(u8Capture & 0x02)
		outp32(CCR1, (inp32(CCR1) & 0xF000F) | (1 << (((u8Capture & 0x01) << 4) + u8IntType)));
	else
		outp32(CCR0, (inp32(CCR0) & 0xF000F) | (1 << (((u8Capture & 0x01) << 4) + u8IntType)));
}
...
/*---------------------------------------------------------------------------------------------------------*/
/* Function: PWM_SetClockSetting		                                                   				   */
/* Parameters:                                                                                             */
/*               eSrcClk 		- [in]		eSYS_EXT/eSYS_APLL/eSYS_UPLL				     			   */
/*               u32PllDiver 	- [in]		1~8	(Only be valid when eSrcClk is eSYS_APLL or eSYS_UPLL)	   */
/*               u32EngineDiver - [in]		1~256											     		   */
/*              											                                        	   */
/* Returns:                                                                                                */
/*               None											                    		               */
/*               							                                                               */
/* Side effects:                                                                                           */
/* Description:                                                                                            */
/*               This function is used to set PWM engine clock source and divier 						   */		
/*---------------------------------------------------------------------------------------------------------*/
BOOL PWM_SetClockSetting(E_SYS_SRC_CLK eSrcClk, UINT32 u32PllDiver, UINT32 u32EngineDiver)
{
	...
	{
		g_u32PwmHz = sysGetPLLOutputHz(eSrcClk, u32Clock) / u32EngineDiver / u32PllDiver;	
...
/*---------------------------------------------------------------------------------------------------------*/
/* Function: PWM_GetEngineClock		                                                   					   */
/*                                                                                                         */
/* Parameters:                                                                                             */
/*               *eSrcClk 		- [out]		Get the Engine clock source (eSYS_EXT/eSYS_APLL/eSYS_UPLL)	   */
/*              											                                        	   */
/* Returns:                                                                                                */
/*               PWM engine Clock (kHz)							                    		       		   */
/*               							                                                               */
/* Side effects:                                                                                           */
/* Description:                                                                                            */
/*               Get Current PWM engine clock															   */		
/*---------------------------------------------------------------------------------------------------------*/
UINT32 PWM_GetEngineClock(E_SYS_SRC_CLK* peSrcClk)
{
	UINT32 u32Source = (inp32(REG_CLKDIV5) & PWM_S) >> 3;
...
VOID PWM_Enable(UINT8 u8Timer,BOOL bEnable)
{
	...
	{
		if(u8Timer & 0x02)
			outp32(CCR1, (inp32(CCR1)& ~(1 << (((u8Timer & 0x01) << 4) + 3))) | (bEnable?(1 << (((u8Timer & 0x01) << 4) + 3)):0));	
		else
			outp32(CCR0, (inp32(CCR0)& ~(1 << (((u8Timer & 0x01) << 4) + 3))) | (bEnable?(1 << (((u8Timer & 0x01) << 4) + 3)):0));
	}
}
...
FLOAT PWM_SetTimerClk(UINT8 u8Timer, PWM_TIME_DATA_T *sPt)
{
	...
	{
		if(u8Timer & 0x02)
			outp32(CCR1, (inp32(CCR1) & ~(1 << (((u8Timer & 0x01) << 4)))) | (sPt->bInverter?(1 << ((u8Timer & 0x01) << 4)):0));
		else
			outp32(CCR0, (inp32(CCR0) & ~(1 << (((u8Timer & 0x01) << 4)))) | (sPt->bInverter?(1 << ((u8Timer & 0x01) << 4)):0));
	}
	else
	...
	{	
		...
		if(u8Timer & 0x02)	
			outp32(PPR, (inp32(PPR) & ~CP1) | (((sPt->u16PreScale - 1) & 0xFF) << 8));
		else	
			outp32(PPR, (inp32(PPR) & ~CP0) | ((sPt->u16PreScale - 1) & 0xFF));
		...
		fFrequency = (FLOAT)g_u32PwmHz / sPt->u16PreScale / sPt->u8ClockSelector / sPt->u32Duty;
	}
	else
	{
		UINT8	u8Divider;
		...
		fFrequency = (FLOAT)  g_u32PwmHz / sPt->fFrequency;		
		...
		fFrequency = ((FLOAT) g_u32PwmHz / u16PreScale / u8Divider) / u16Duty;	
