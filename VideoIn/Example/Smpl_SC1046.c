/**************************************************************************//**
 * @file     Smpl_SC1046.c
 * @brief    Initialize SC1046 sample code
 *
...
#define CHIP_VERSION_REG	0x3008		//==>read back will same as I2C slave address
#define SC1046_SLAVE		0x60
...
struct NT_RegTable g_SC1046_InitTable[] =
{
	{g_sSC1046_HD720,_REG_TABLE_SIZE(g_sSC1046_HD720)},		
...
VOID SC1046_Init(UINT32 nIndex, UINT32 u32Resolution)
{
	UINT32 u32Idx;
	UINT32 u32TableSize;
	UINT8  u8DeviceID;
	...
	sysDelay(2);
	u32TableSize = g_SC1046_InitTable[0].uTableSize;
	psRegValue = g_SC1046_InitTable[0].sRegTable;
	...
	{		
		...
		if(u8Data!= (psRegValue->u8Value))
			sysprintf("Error write sensor register 1\n");
	}
	id0 = I2C_Read_8bitSlaveAddr_16bitReg_8bitData(u8DeviceID,CHIP_VERSION_REG);
	//static UINT8 I2C_Read_8bitSlaveAddr_16bitReg_8bitData(UINT8 uAddr, UINT16 uRegAddr)
	sysprintf("Detectd sensor ID=%0x\n",id0);
...
UINT32 Smpl_SC1046_HD(UINT8* pu8FrameBuffer0, UINT8* pu8FrameBuffer1, UINT8* pu8FrameBuffer2)
{
	...
	pVin->Open(72000, 16000);
#endif

	SC1046_Init(0, REG_VALUE_HD720);			
	...
	pVin->SetDataFormatAndOrder(eVIDEOIN_IN_UYVY, 				//SC1046
