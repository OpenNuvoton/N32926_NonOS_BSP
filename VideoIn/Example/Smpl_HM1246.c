/**************************************************************************//**
 * @file     Smpl_HM1246.c
 * @brief    Initialize HM1246 sample code
 *
...
#undef REG_VALUE_SXGA
#define REG_VALUE_INIT      0
#define REG_VALUE_VGA       1
#define REG_VALUE_HD720P    2
#define REG_VALUE_SXGA      3   //1280X720
...
struct HM_RegTable g_HM1246_InitTable[] =
{
    {g_sHM1246_INIT,_REG_TABLE_SIZE(g_sHM1246_INIT)},
    {g_sHM1246_VGA,_REG_TABLE_SIZE(g_sHM1246_VGA)},
    {g_sHM1246_HD720P,_REG_TABLE_SIZE(g_sHM1246_HD720P)},
    {g_sHM1246_SXGA,_REG_TABLE_SIZE(g_sHM1246_SXGA)},
    {0,0}
};
...
VOID HM1246_Init(UINT32 nIndex, UINT32 u32Resolution)
{
    UINT32 u32Idx;
    UINT32 u32TableSize;
    UINT8  u8DeviceID;
    UINT8 id0, id1;
    static UINT32 u32SensorInit = 0;
    ...
    if( u32Resolution < REG_VALUE_SXGA )
    {
        //Init, VGA and HD720P is released by Himax, After initialization table, the senosr can be change resolution by filled HD720P or VGA sesnor only.


        if(u32SensorInit == 0)
        {
            u32TableSize = g_HM1246_InitTable[0].uTableSize;                //Init
            psRegValue = g_HM1246_InitTable[0].sRegTable;
            u8DeviceID = g_uOvDeviceID[nIndex];
            for(u32Idx=0; u32Idx<u32TableSize; u32Idx++, psRegValue++)
            {
                //Default init table is VGA resolution
        ...
        u32TableSize = g_HM1246_InitTable[u32Resolution].uTableSize;    //Target Resolution
        psRegValue = g_HM1246_InitTable[u32Resolution].sRegTable;
        u8DeviceID = g_uOvDeviceID[nIndex];
        for(u32Idx=0; u32Idx<u32TableSize; u32Idx++, psRegValue++)
        {
            //Resolution
    ...
    {
        //SXGA reolution inlude init table.
        u32TableSize = g_HM1246_InitTable[u32Resolution].uTableSize;    //Target Resolution
        psRegValue = g_HM1246_InitTable[u32Resolution].sRegTable;
        u8DeviceID = g_uOvDeviceID[nIndex];
        for(u32Idx=0; u32Idx<u32TableSize; u32Idx++, psRegValue++)
...
UINT32 Smpl_HM1246_VGA(UINT8* pu8FrameBuffer0, UINT8* pu8FrameBuffer1, UINT8* pu8FrameBuffer2)
{
...
#endif
    pVin->Open(72000, 24000);   /* Sensor clock 24MHz--> PCLK = 78 ~ 84 MHz, 30f/s */
    //pVin->Open(72000, 16000); /* Sensor clock 14.79MHz--> PCLK = 48~52 MHz */
    HM1246_Init(9, REG_VALUE_VGA);
    ...
    pVin->SetDataFormatAndOrder(eVIDEOIN_IN_VYUY,       //2
...
UINT32 Smpl_HM1246_HD720P(UINT8* pu8FrameBuffer0, UINT8* pu8FrameBuffer1, UINT8* pu8FrameBuffer2)
{
    ...
    pVin->Open(72000, 24000);   /* Sensor clock 24MHz--> PCLK = 78 ~ 84 MHz, 30f/s */
    //pVin->Open(72000, 16000); /* Sensor clock 14.79MHz--> PCLK = 48~52 MHz */
    HM1246_Init(9, REG_VALUE_HD720P);
    ...
    pVin->SetDataFormatAndOrder(eVIDEOIN_IN_VYUY,       //2
                                eVIDEOIN_IN_YUV422  ,   //Intput format
                                eVIDEOIN_OUT_YUV422);       //Output format for packet
    pVin->SetCropWinStartAddr(2,                            //Horizontal start position
                              2);                     //Useless
