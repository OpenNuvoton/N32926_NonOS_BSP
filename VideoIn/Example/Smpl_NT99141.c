/**************************************************************************//**
 * @file     Smpl_NT99141.c
 * @brief    Initialize NT99141 sample code
 *
...
UINT32 Smpl_NT99141_VGA_MotionDetection(UINT8* pu8FrameBuffer0, UINT8* pu8FrameBuffer1, UINT8* pu8FrameBuffer2)
{
    ...
    NT99141_Init(8, REG_VALUE_VGA);
    pVin->EnableInt(eVIDEOIN_VINT);
    pVin->EnableInt(eVIDEOIN_MDINT);
    pVin->InstallCallback(eVIDEOIN_VINT,
                          (PFN_VIDEOIN_CALLBACK)VideoIn_InterruptHandler,
                          &pfnOldCallback );  //Frame End interrupt

    pVin->InstallCallback(eVIDEOIN_MDINT,
                          (PFN_VIDEOIN_CALLBACK)MD_Handler,
                          &pfnOldCallback );  //Frame End interrupt
    ...
    pVin->SetMotionDet(TRUE,
                       FALSE,
                       FALSE);

    pVin->SetMotionDetEx(20,
                         (UINT32)u8MDO,
                         (UINT32)u8MDY);
...
//#ifdef __2ND_PORT__
    i32ErrCode = register_vin_device(2, &Vin2);
    if(i32ErrCode<0)
    {
        sysprintf("Register vin 1 device fail\n");
        return (UINT32)-1;
...
//#endif
    ...
    pVin2->Open(48000, 24000);  /* Sensor clock 24MHz--> PCLK = 72MHz */
    //NT99141_Init(8, REG_VALUE_HD720);

    pVin2->EnableInt(eVIDEOIN_VINT);
    pVin2->InstallCallback(eVIDEOIN_VINT,
                           (PFN_VIDEOIN_CALLBACK)VideoIn2_InterruptHandler,
                           &pfnOldCallback );  //Frame End interrupt
    pVin2->SetPacketFrameBufferControl(FALSE, FALSE);


    pVin2->SetDataFormatAndOrder(eVIDEOIN_IN_UYVY,              //NT99050
                                 eVIDEOIN_IN_YUV422  ,   //Intput format
                                 eVIDEOIN_OUT_YUV422);       //Output format for packet
    pVin2->SetCropWinStartAddr(0,                           //Horizontal start position
                               0);                     //Useless

    pVin2->SetStandardCCIR656(TRUE);                        //standard CCIR656 mode
    pVin2->SetSensorPolarity(FALSE,
                             FALSE,
                             FALSE);
    pVin2->SetCropWinSize(OPT_CROP_HEIGHT,              //UINT16 u16Height,
                          OPT_CROP_WIDTH);                //UINT16 u16Width

    pVin2->PreviewPipeSize(OPT_PREVIEW_HEIGHT, OPT_PREVIEW_WIDTH);
    pVin2->EncodePipeSize(OPT_ENCODE_HEIGHT, OPT_ENCODE_WIDTH);
#ifdef __TV__
    pVin2->SetStride(OPT_STRIDE, OPT_ENCODE_WIDTH_2);
    pVin2->SetBaseStartAddress(eVIDEOIN_PACKET, 0, (UINT32)((UINT32)pu8FrameBuffer0) );
#else
    pVin2->SetStride(OPT_STRIDE, OPT_ENCODE_WIDTH_2);
    pVin2->SetBaseStartAddress(eVIDEOIN_PACKET, (E_VIDEOIN_BUFFER)0, (UINT32)((UINT32)pu8FrameBuffer0 + (OPT_STRIDE-OPT_PREVIEW_WIDTH)/2*2) );
#endif
    pVin2->SetBaseStartAddress(eVIDEOIN_PLANAR,
                               (E_VIDEOIN_BUFFER)0,                            //Planar buffer Y addrress
                               (UINT32)u8PlanarFrameBuffer2_0);

    pVin2->SetBaseStartAddress(eVIDEOIN_PLANAR,
                               (E_VIDEOIN_BUFFER)1,                            //Planar buffer U addrress
                               (UINT32)u8PlanarFrameBuffer2_0+OPT_ENCODE_WIDTH_2*OPT_ENCODE_HEIGHT_2);

    pVin2->SetPlanarFormat(eVIDEOIN_PLANAR_YUV422);             // Planar YUV422/420/macro
    pVin2->SetBaseStartAddress(eVIDEOIN_PLANAR,
                               (E_VIDEOIN_BUFFER)2,                            //Planar buffer V addrress
                               (UINT32)u8PlanarFrameBuffer2_0+OPT_ENCODE_WIDTH_2*OPT_ENCODE_HEIGHT_2+OPT_ENCODE_WIDTH_2*OPT_ENCODE_HEIGHT_2/2);

#ifdef _MACRO_BLOCK_
    pVin2->SetPlanarFormat(eVIDEOIN_MACRO_PLANAR_YUV420);       // Planar YUV422/420/macro
    pVin2->SetBaseStartAddress(eVIDEOIN_PLANAR,
                               2,                          //Planar buffer V addrress
                               (UINT32)u8PlanarFrameBuffer2_0+OPT_ENCODE_WIDTH_2*OPT_ENCODE_HEIGHT_2);
    ...
    pVin2->SetPipeEnable(TRUE,                                  // Engine enable?
                         eVIDEOIN_BOTH_PIPE_ENABLE);     // which packet was enabled.
