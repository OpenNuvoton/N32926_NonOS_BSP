/**************************************************************************//**
 * @file     Smpl_HM2056.c
 * @brief    Initialize HM2056 sample code
 *
...
{
	{g_sHM2056_VGA,_REG_TABLE_SIZE(g_sHM2056_VGA)},		
	{g_sHM2056_HD720,_REG_TABLE_SIZE(g_sHM2056_HD720)},		
	{0,0}
};
...
VOID HM2056_Init(UINT32 nIndex, UINT32 u32Resolution)
{
	UINT32 u32Idx;
	UINT32 u32TableSize;
	UINT8  u8DeviceID;
	...
	id0 = I2C_Read_8bitSlaveAddr_16bitReg_8bitData(u8DeviceID,CHIP_ID_H);		//0x20
	id1 = I2C_Read_8bitSlaveAddr_16bitReg_8bitData(u8DeviceID,CHIP_ID_L);		//0x56
	sysprintf("Detectd sensor id0=%0x id1=%02x\n",id0, id1);
	
	id0 = I2C_Read_8bitSlaveAddr_16bitReg_8bitData(u8DeviceID,CHIP_VID);		//MFG
	sysprintf("Detectd VID =%0x\n",id0);
	
	u32TableSize = g_HM2056_InitTable[u32Resolution].uTableSize;
	psRegValue = g_HM2056_InitTable[u32Resolution].sRegTable;
...
BOOL CMOSHM2056_SetLightMode(LIGHTMODE a_eLightMode)
{
	switch(a_eLightMode)
	{
...
// Frame Rate	10FPS+ 2016.12.13
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x038F, 0x07)) 
	                return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0390, 0x5C)) 
	                return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x03A4, 0x07)) 
	                return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x03A5, 0x5C)) 
	                return FALSE;

// Color Saturation 2017.08.07
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0480, 0x80))
          		return FALSE;
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0481, 0x00)) 
	                return FALSE;

// Sharpness Strength 2017.02.27
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x069C, 0x18))
          		return FALSE;

// Brightness 2017.08.07
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x04C0, 0x90))
          		return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0381, 0x3A))
          		return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0382, 0x26))
          		return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x038E, 0x30))
          		return FALSE;	
// Update Register
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0000, 0x01))
          		return FALSE;
		break;
		case eLIGHTSEN_NIGHT:

//
//	NIGHT MODE
//

// Fram Rate	5FPS+ 2017.03.09
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x038F, 0x0E)) 
	                return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0390, 0xB8)) 
	                return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x03A4, 0x0E)) 
	                return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x03A5, 0xB8)) 
	                return FALSE;	

// Color Saturation
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0480, 0x00))
          		return FALSE;
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0481, 0x00)) 
	                return FALSE;	

// Sharpness Strength 2017.02.27
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x069C, 0x22))
          		return FALSE;

// Brightness 2017.01.18
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x04C0, 0x80))
          		return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0381, 0x3A))
          		return FALSE;	
	if(!I2C_Write_8bitSlaveAddr_16bitReg_8bitData(0x48,0x0382, 0x26))
...
{
	static UINT8 u8LightMode = 1;
	if(u8LightMode == 0)
	{
		CMOSHM2056_SetLightMode(eLIGHTSEN_NIGHT);
		u8LightMode = 1;
	}
	else
	{
		CMOSHM2056_SetLightMode(eLIGHTSEN_DAY);
		u8LightMode = 0;
	}
...
UINT32 Smpl_HM2056_VGA(UINT8* pu8FrameBuffer0, UINT8* pu8FrameBuffer1, UINT8* pu8FrameBuffer2)
{
	...
	pVin->Open(72000, 16000);	/* Sensor clock 16MHz--> PCLK = MHz */
	HM2056_Init(9, REG_VALUE_VGA);			
