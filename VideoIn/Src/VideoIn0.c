...
#define REAL_CHIP
PFN_VIDEOIN_CALLBACK (pfnvideoIn0_IntHandlerTable)[4]={0};

/*---------------------------------------------------------------------------------------------------------*/
/* Function: videoIn0_IntHandler                                                                             */
/*                                                                                                         */
/* Parameters:                                                                                             */
/*      None                                                                                               */
/*                                                                                                         */
	...
	)
{
	...
	{//Check VideoIn0 Enable 
		...
		{
			if(pfnvideoIn0_IntHandlerTable[0]!=0)	
				pfnvideoIn0_IntHandlerTable[0](uBuf, uBuf, bIsFiled, u32EscapeFrame);
		...
		{
			if(pfnvideoIn0_IntHandlerTable[1]!=0)	
				pfnvideoIn0_IntHandlerTable[1](uBuf, uBuf, bIsFiled, u32EscapeFrame);
			outp32(REG_VPEINT, (u32CapInt & ~(MDINT | VINT | MEINT)));			/* Clear Address match interrupt */
		}	
		else if ((u32CapInt & (MEINTEN|MEINT)) == (MEINTEN|MEINT))
		{
			if(pfnvideoIn0_IntHandlerTable[2]!=0)	
				pfnvideoIn0_IntHandlerTable[2](uBuf, uBuf, bIsFiled, u32EscapeFrame);	
		...
		{
			if(pfnvideoIn0_IntHandlerTable[3]!=0)	
				pfnvideoIn0_IntHandlerTable[3](uBuf, uBuf, bIsFiled, u32EscapeFrame);	
...
)
{
	...
	outp32(REG_AHBIPRST, inp32(REG_AHBIPRST) | VIN0_RST);			/* Global reset Capture */
	outp32(REG_AHBIPRST, inp32(REG_AHBIPRST) & (~VIN0_RST));								
	u32PllClock = sysGetPLLOutputHz(eSYS_UPLL, u32ExtFreq);
	...
	outp32(REG_CLKDIV4, (inp32(REG_CLKDIV4) | BIT11) & ~VIN0_N);	/* CAP0 engine clock divider need to be filled with 0 */		
...
/*                                                                                                         */
/*---------------------------------------------------------------------------------------------------------*/
void videoIn0_SetPacketFrameBufferControl(
	BOOL bFrameSwitch,
...
void 
videoIn0_GetPacketFrameBufferControl(
	PBOOL pbFrameSwitch,
...
{		
	...
	{
		*pfnOldCallback = pfnvideoIn0_IntHandlerTable[0];
		pfnvideoIn0_IntHandlerTable[0] = (PFN_VIDEOIN_CALLBACK)(pfnCallback);
	}	
	else if(eIntType == eVIDEOIN_ADDRMINT)
	{
		*pfnOldCallback = pfnvideoIn0_IntHandlerTable[1];
		pfnvideoIn0_IntHandlerTable[1] = (PFN_VIDEOIN_CALLBACK)(pfnCallback);
	}
	else if(eIntType == eVIDEOIN_MEINT)
	{
		*pfnOldCallback = pfnvideoIn0_IntHandlerTable[2];
		pfnvideoIn0_IntHandlerTable[2] = (PFN_VIDEOIN_CALLBACK)(pfnCallback);
	}	
	else if(eIntType == eVIDEOIN_MDINT)
	{
		*pfnOldCallback = pfnvideoIn0_IntHandlerTable[3];
		pfnvideoIn0_IntHandlerTable[3] = (PFN_VIDEOIN_CALLBACK)(pfnCallback);
	}	
	else
...
/*---------------------------------------------------------------------------------------------------------*/
void 
videoIn0_SetDataFormatAndOrder(
...
void videoIn0_GetDataFormatAndOrder(
...
void videoIn0_SetMotionDetFreq(UINT32 u32DetFreq)
{
	outp32(REG_VPEMD+g_u32PortOffset, (inp32(REG_VPEMD+g_u32PortOffset) & ~MDDF) |
				((u32DetFreq <<10) & MDDF) );
}	
void videoIn0_GetMotionDetFreq(PUINT32 pu32DetFreq)
{
	UINT32 u32RegData;
	u32RegData = inp32(REG_VPEMD+g_u32PortOffset); 
	*pu32DetFreq = u32RegData & MDDF;
...
UINT32 
videoIn0_GetProcessedDataCount(    // Read Only
...
ERRCODE 
videoIn0_SetVerticalScaleFactor(
...
ERRCODE 
DrvVideoIn0_GetVerticalScaleFactor(
	E_VIDEOIN_PIPE ePipe,
...
ERRCODE 
videoIn0_SetHorizontalScaleFactor(
...
ERRCODE 
DrvVideoIn0_GetHorizontalScaleFactor(
	E_VIDEOIN_PIPE bPipe,
	PUINT16 pu16Numerator, 
	PUINT16 pu16Denominator
	)
...
void 
DrvVideoIn0_SetFrameRateScaleFactor(
	UINT8 u8Numerator, 
	UINT8 u8Denominator
	)
{
...
void 
DrvVideoIn0_GetFrameRateScaleFactor(
	PUINT8 pu8Numerator, 
...
/*                                                                                                         						*/
/*---------------------------------------------------------------------------------------------------------*/
void videoIn0_SetStandardCCIR656(BOOL bIsStandard)
{
	if(bIsStandard==TRUE)
...
void videoIn0_GetFieldDetection(
...
/*                                                                                                         	*/
/*----------------------------------------------------------------------------------------------------------*/
ERRCODE videoIn0_SetColorEffect(
...
void DrvVideoIn0_GetColorEffect(
...
ERRCODE 
videoIn0_SetColorEffectParameter(
	UINT8 	u8YComp,
	UINT8  	u8UComp,
	UINT8 	u8VComp
	)
...
ERRCODE 
videoIn0_GetColorEffectParameter(
	PUINT8 	pu8YComp,
	PUINT8  	pu8UComp,
	PUINT8 	pu8VComp
	)
...
/* ===================================================================================================================================== */
INT32 videoIn0_PreviewPipeSize(UINT16 u16height, UINT16 u16width)
{
	INT32 i32ErrCode;
	UINT32 u32cropwidth,  u32cropheight;
	
	videoIn0_GetCropWinSize(&u32cropheight, &u32cropwidth);	
	i32ErrCode = videoIn0_SetVerticalScaleFactor(eVIDEOIN_PACKET, u16height, u32cropheight);
	if(i32ErrCode!=Successful)
		return i32ErrCode;
	i32ErrCode = videoIn0_SetHorizontalScaleFactor(eVIDEOIN_PACKET, u16width, u32cropwidth);
	return Successful;	
}
INT32 videoin0_EncodePipeSize(UINT16 u16height, UINT16 u16width)
{
	INT32 i32ErrCode;
	UINT32 u32cropwidth,  u32cropheight;
	videoIn0_GetCropWinSize(&u32cropheight, &u32cropwidth);	
	i32ErrCode = videoIn0_SetVerticalScaleFactor(eVIDEOIN_PLANAR, u16height, u32cropheight);
	if(i32ErrCode != Successful)
		return i32ErrCode;
	i32ErrCode = videoIn0_SetHorizontalScaleFactor(eVIDEOIN_PLANAR, u16width, u32cropwidth);
	return i32ErrCode;		
}

VINDEV_T nvt_vin0 =
{
	videoIn0_Init,				 	// void (*Init)(BOOL bIsEnableSnrClock, E_VIDEOIN_SNR_SRC eSnrSrc, UINT32 u32SensorFreqKHz, E_VIDEOIN_DEV_TYPE eDevType):
	videoIn0_Open,					// INT32 (*Open)(UINT32 u32EngFreqKHz, UINT32 u32SensorFreqKHz);
	videoIn0_Close,				// void (*Close)(void);
	videoIn0_SetPipeEnable,			// void (*SetPipeEnable)(BOOL bEngEnable, E_VIDEOIN_PIPE ePipeEnable);
	videoIn0_SetPlanarFormat,		// void (*SetPlanarFormat)(E_VIDEOIN_PLANAR_FORMAT ePlanarFmt);
	videoIn0_SetCropWinSize,			// void (*SetCropWinSize)(UINT32 u32height, UINT32 u32width);
	videoIn0_SetCropWinStartAddr,		// void (*SetCropWinStartAddr)(UINT32 u32VerticalStart, UINT32 u32HorizontalStart);
	videoIn0_PreviewPipeSize,			// INT32 (*PreviewPipeSize)(UINT16 u32height, UINT16 u32width);
	videoin0_EncodePipeSize,			// INT32 (*EncodePipeSize)(UINT16 u32height, UINT16 u32width);
	videoIn0_SetStride,				// void (*SetStride)(UINT32 u16packetstride, UINT32 u32planarstride);  								
	videoIn0_GetStride, 				// void (*GetStride)(PUINT32	pu32PacketStride, PUINT32 pu32PlanarStride);
	videoIn0_EnableInt,				// INT32 (*EnableInt)(E_VIDEOIN_INT_TYPE eIntType);
	videoIn0_DisableInt,				// INT32 (*DisableInt)(E_VIDEOIN_INT_TYPE eIntType);
	videoIn0_InstallCallback,			// INT32 (*InstallCallback)(E_VIDEOIN_INT_TYPE eIntType, PFN_VIDEOIN_CALLBACK pfnCallback, PFN_VIDEOIN_CALLBACK *pfnOldCallback);
	videoIn0_SetBaseStartAddress,		// INT32 (*SetBaseStartAddress(E_VIDEOIN_PIPE ePipe, E_VIDEOIN_BUFFER eBuf, UINT32 u32BaseStartAddr);					
	videoIn0_SetOperationMode,		// void (*SetOperationMode(BOOL bIsOneSutterMode)			
	videoIn0_GetOperationMode,		// BOOL (*GetOperationMode)(void);
	videoIn0_SetPacketFrameBufferControl, 	// void (*videoIn1_SetPacketFrameBufferControl)(BOOL bFrameSwitch, BOOL bFrameBufferSel);
	videoIn0_SetSensorPolarity,		// void (*videoIn1_SetSensorPolarity)(BOOL bVsync, BOOL bHsync,  BOOL bPixelClk);
	videoIn0_SetColorEffectParameter,	// INT32 SetColorEffectParameter(UINT8 u8YComp, UINT8 u8UComp, UINT8 u8VComp);
	videoIn0_SetDataFormatAndOrder, 	// void (*SetDataFormatAndOrder)(E_VIDEOIN_ORDER eInputOrder, E_VIDEOIN_IN_FORMAT eInputFormat, E_VIDEOIN_OUT_FORMAT eOutputFormat)
	videoIn0_SetMotionDet,			// void (*SetMotionDet)(BOOL bEnable, BOOL bBlockSize,	BOOL bSaveMode);
	videoIn0_SetMotionDetEx,			// void (*SetMotionDetEx)(UINT32 u32Threshold, UINT32 u32OutBuffer, UINT32 u32LumBuffer)
	videoIn0_SetInputType,			// void SetInputType(UINT32 u32FieldEnable, E_VIDEOIN_TYPE eInputType,	BOOL bFieldSwap);
	videoIn0_SetStandardCCIR656,		// void(*SetStandardCcir656)(BOOL);
	videoIn0_SetShadowRegister,		// void(*SetShadowRegister)(void);

	videoIn0_SetFieldDetection,		// void(*SetFieldDetection)(BOOL bDetPosition,BOOL bFieldDetMethod);
	videoIn0_GetFieldDetection              // void(*GetFieldDetection)(PBOOL pbDetPosition,PBOOL pbFieldDetMethod);
};
